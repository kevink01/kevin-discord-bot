...
class ExtendedClient extends Client {
  ...
  private async readCommand(dir: string) {
    printLoad(
      this.load++,
      Direction.left,
      `Loading ${dir.substring(dir.lastIndexOf("\\") + 1)}`
    );
    ...
    for (const file of files) {
      ...
      if (stats.isDirectory()) {
        ...
        printLoad(
          this.load,
          Direction.left,
          `✅ Registring command: ${command.name} ${aliases}`
        );
      }
    }
    printLoad(
      --this.load,
      Direction.right,
      `❗ Done loading ${dir.substring(dir.lastIndexOf("\\") + 1)}`
    );
  }
    ...
    for (const file of files) {
      ...
      if (stats.isDirectory()) {
        ...
        printLoad(
          this.load,
          Direction.left,
          `✅ Registring slash command: ${slashcommand.name}`
        );
      }
    }
    printLoad(
