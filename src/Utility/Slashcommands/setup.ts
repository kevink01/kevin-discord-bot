...
export const selectionRow: ActionRowBuilder<ButtonBuilder> = new ActionRowBuilder<ButtonBuilder>().addComponents([
	new ButtonBuilder().setCustomId('default').setLabel('Default').setEmoji('❗').setStyle(ButtonStyle.Success),
	new ButtonBuilder().setCustomId('create').setLabel('Create').setEmoji('➕').setStyle(ButtonStyle.Secondary),
	new ButtonBuilder().setCustomId('find').setLabel('Find').setEmoji('🔍').setStyle(ButtonStyle.Primary),
	new ButtonBuilder().setCustomId('skip').setLabel('Skip').setEmoji('⏩').setStyle(ButtonStyle.Danger)
]);
 ...
 */
export async function startSetup(embed: EmbedBuilder, interaction: ButtonInteraction): Promise<void> {
	/* Row for choosing action for a completed setup */
	const editConfirmRow: ActionRowBuilder<ButtonBuilder> = new ActionRowBuilder<ButtonBuilder>().addComponents([
		new ButtonBuilder().setCustomId('confirm').setLabel('Confirm').setEmoji('✅').setStyle(ButtonStyle.Success),
		new ButtonBuilder().setCustomId('edit').setLabel('Edit setup').setEmoji('📝').setStyle(ButtonStyle.Primary),
		new ButtonBuilder().setCustomId('cancel').setLabel('Cancel setup').setEmoji('❌').setStyle(ButtonStyle.Danger)
	]);
	...
	for (let j = 0; j < Object.keys(Setup).length; j++) {
		switch (j) {
			...
			case 1:
				...
				await chooseChannel(embed, interaction, j === 0 ? Setup.welcome : Setup.log)
					.then((value: string) => {
						if (j === 0) {
			...
			case 3:
				...
				await chooseRole(embed, interaction, j === 2 ? Setup.member : Setup.muted)
					.then((value: string) => {
						if (j === 2) {
	...
	embed.setTitle('Do you want to confirm your selection?').setDescription(getChoices()).setFields([]);
	await interaction
		...
		.then(async () => {
			...
			collector.on('collect', async (i: ButtonInteraction) => {
				switch (i.customId) {
					case 'confirm':
						...
						await confirmSetup(embed, interaction)
							.then(async () => {
								collector.stop();
								await delay(2000);
								embed.setTitle('Finished setup!').setDescription('Goodbye!').setColor('Green').setFields([]);
								await interaction.editReply({ components: [], embeds: [embed] }).then(() => {
									delay(2000)
										.then(async () => {
											await interaction
												.deleteReply()
					...
					case 'cancel':
						/* Cancel the setup */
						await cancelSetup(embed, interaction).then(async (value: boolean) => {
							collector.stop();
							/* If we cancel, unsaved progress is deleted */
							if (value === true) {
								embed.setTitle('Cancelling setup...').setDescription('Goodbye').setColor('Red').setFields([]);
								await interaction
 ...
 * @param interaction : Interaction to control
 */
export async function confirmSetup(embed: EmbedBuilder, interaction: ButtonInteraction): Promise<void> {
	...
	for (const [key, value] of Object.entries(choices) as [string, string][]) {
		if (value !== undefined && value !== 'skip') {
			switch (key) {
				case 'welcomeChannel':
					selections.push('**Welcome channel**: ' + (value.startsWith('find') ? value.split(' ')[1] : value));
					break;
				case 'logChannel':
					selections.push('**Log channel**: ' + (value.startsWith('find') ? value.split(' ')[1] : value));
					break;
				case 'memberRole':
					selections.push('**Member role**: ' + (value.startsWith('find') ? value.split(' ')[1] : value));
					break;
				case 'muteRole':
					selections.push('**Muted role**: ' + (value.startsWith('find') ? value.split(' ')[1] : value));
					break;
				default:
					break;
			}
		...
		.then(async () => {
			if (choices.welcomeChannel) {
				if (choices.welcomeChannel === 'default') {
					...
					interaction.guild.channels
						.create({
							...
							permissionOverwrites: [
								{
									id: interaction.guild.roles.cache.find((r: Role) => r.name === '@everyone').id,
									allow: ['ViewChannel'],
						...
						.then((channel: GuildTextBasedChannel) => {
							(interaction.client as ExtendedClient).config.defaults.welcomeChannel = channel;
					...
					const channel: GuildBasedChannel = interaction.guild.channels.cache.find(
						(c: GuildTextBasedChannel) =>
							c.id === choices.welcomeChannel.split(' ')[2] && c.type === ChannelType.GuildText
					);
					if (!channel) {
						status[statusCounter] = '❌';
						embed.setDescription(
						...
						(interaction.client as ExtendedClient).config.defaults.welcomeChannel = channel as GuildTextBasedChannel;
						status[statusCounter] = '✅';
						embed.setDescription(
							status
								.map((value, index) => {
			...
			if (choices.logChannel) {
				if (choices.logChannel === 'default') {
					...
					interaction.guild.channels
						...
						.then((channel: GuildTextBasedChannel) => {
							(interaction.client as ExtendedClient).config.defaults.logChannel = channel;
							status[statusCounter] = '✅';
							embed.setDescription(
								status
									.map((value, index) => {
					...
					const channel: GuildBasedChannel = interaction.guild.channels.cache.find(
						(c: GuildTextBasedChannel) => c.id === choices.logChannel.split(' ')[2] && c.type === ChannelType.GuildText
					);
					if (!channel) {
						status[statusCounter] = '❌';
						embed.setDescription(
						...
						(interaction.client as ExtendedClient).config.defaults.logChannel = channel as GuildTextBasedChannel;
						status[statusCounter] = '✅';
						embed.setDescription(
							status
								.map((value, index) => {
			...
			if (choices.memberRole) {
				...
				if (choices.memberRole === 'default') {
					...
					interaction.guild.roles
						...
						.then((r: Role) => {
							role = r;
							(interaction.client as ExtendedClient).config.defaults.memberRole = r;
							roleStatus = true;
						})
						.catch((err) => {
							status[statusCounter] = '❌';
					...
					if (!findRole) {
						...
						role = findRole;
						(interaction.client as ExtendedClient).config.defaults.memberRole = findRole;
						roleStatus = true;
					}
				}
              ...
              */
                      ...
                      */
										c.permissionOverwrites
											.edit(
												c.id !== verified.id
													? interaction.guild.roles.cache.find((r: Role) => r.name === '@everyone')
													: role,
												{
													ViewChannel: false
												}
			...
			if (choices.muteRole) {
				...
				if (choices.muteRole === 'default') {
					...
					interaction.guild.roles
						...
						.then((r: Role) => {
							role = r;
							(interaction.client as ExtendedClient).config.defaults.muteRole = r;
							roleStatus = true;
						})
						.catch((err) => {
							status[statusCounter] = '❌';
					...
					if (!findRole) {
						...
						role = findRole;
						(interaction.client as ExtendedClient).config.defaults.muteRole = findRole;
						roleStatus = true;
					}
				}
				while (!role && !roleStatus) {
 ...
 */
export async function editSetup(embed: EmbedBuilder, interaction: ButtonInteraction, edit: boolean): Promise<void> {
	/* Row for making edits */
	const buttonFilter = (i: any) => i.user.id === interaction.user.id;
	const confirmRow: ActionRowBuilder<ButtonBuilder> = new ActionRowBuilder<ButtonBuilder>().addComponents([
		new ButtonBuilder().setCustomId('confirm').setLabel('Confirm').setEmoji('✅').setStyle(ButtonStyle.Success)
	]);
	/* Add all configuration options */
	confirmRow.addComponents(
		Object.entries(Setup).map(([str, value]: [string, Setup]) => {
			return new ButtonBuilder().setCustomId(value).setLabel(value).setStyle(ButtonStyle.Primary);
		})
	);
	// TODO: Menus?
	let editFirstResponse: boolean = false; /* If we are editing current setup, only reset choices ONCE */
 ...
 * @returns
 */
export async function handleError(embed: EmbedBuilder, interaction: ButtonInteraction, err: any): Promise<void> {
	try {
		console.error(err);
		embed.setTitle('Something went wrong').setDescription('Cancelling setup').setColor('Red').setFields([]);
 ...
 * @returns
 */
async function cancelSetup(embed: EmbedBuilder, interaction: ButtonInteraction): Promise<boolean> {
	const buttonFilter = (i: any) => i.user.id === interaction.user.id;
	let response: boolean = false;

	/* Yes & no buttons */
	const confirmRow: ActionRowBuilder<ButtonBuilder> = new ActionRowBuilder<ButtonBuilder>().addComponents([
		new ButtonBuilder().setCustomId('yes').setLabel('Yes').setEmoji('✅').setStyle(ButtonStyle.Danger),
		new ButtonBuilder().setCustomId('no').setLabel('No').setEmoji('❌').setStyle(ButtonStyle.Success)
	]);
 ...
 */
async function chooseChannel(embed: EmbedBuilder, interaction: ButtonInteraction, choice: string): Promise<string> {
	let response: string = '';
	let answer: boolean = false;
	/* If creating a channel, listen to user's messages */
	const messageFilter = async (m: Message) => m.author.id === interaction.user.id;
	const buttonFilter = (i: any) => i.user.id === interaction.user.id;
	switch (choice) {
		case Setup.welcome:
			embed
				...
				.setFields([
					{
						name: 'Warning',
						value: 'No welcome channel will make system channel the welcome channel'
					}
				]);
			break;
		case Setup.log:
	...
	await interaction.editReply({ components: [selectionRow], embeds: [embed] }).catch((err) => {
		throw new Error(err);
	});
	/* Listen to first button click */
	const collector = interaction.channel.createMessageComponentCollector({
	...
	collector.on('collect', async (i: ButtonInteraction) => {
		switch (i.customId) {
			case 'default':
				...
				embed
					.setTitle(`${choice === Setup.welcome ? 'Welcome' : 'Logs'} channel selection`)
						...
						await interaction.channel
							...
							.then(async (collection: Collection<string, Message>) => {
								...
								embed
									...
									.setColor('Yellow')
									.setFields([{ name: 'Channel name', value: content, inline: true }]);
			...
			case 'find':
				...
				selectMenu.setComponents(
					new StringSelectMenuBuilder()
						...
						.setPlaceholder(choice === Setup.welcome ? 'Select welcome channel' : 'Select log channel')
						.setOptions(
				...
				await interaction
					...
					.then(async () => {
						...
						channelCollector.on('collect', async (i: StringSelectMenuInteraction) => {
							...
							embed
								...
								.setDescription('find')
								.setColor('Yellow')
								.setFields([{ name: 'Channel name', value: selection, inline: true }]);
 ...
 * @returns             : String choice
 */
async function chooseRole(embed: EmbedBuilder, interaction: ButtonInteraction, choice: string): Promise<string> {
	let response: string = '';
	let answer: boolean = false;
	...
	switch (choice) {
		case Setup.member:
			...
			break;
		case Setup.muted:
			embed.setTitle(Setup.muted).setDescription('Do you want to setup a mute role?').setColor('Green').setFields([]);
			break;
		default:
			break;
	}
	...
	collector.on('collect', async (i: ButtonInteraction) => {
		switch (i.customId) {
			case 'default':
				...
				embed
					.setTitle(`${choice === Setup.member ? 'Member' : 'Muted'} role selection`)
					.setDescription('default')
					.setColor('Yellow')
					.setFields([
						{
						...
						await interaction.channel
							...
							.then(async (collection: Collection<string, Message>) => {
								...
								embed
									...
									.setDescription('create')
									.setColor('Yellow')
									.setFields([{ name: 'Member name', value: content, inline: true }]);
								response = 'create ' + content;
								await interaction
									.editReply({
										components: [],
			...
			case 'find':
				...
				selectMenu.setComponents(
					new StringSelectMenuBuilder()
						.setCustomId('roles')
						.setPlaceholder(choice === Setup.member ? 'Select member role' : 'Select mute role')
						.setOptions(
				...
				await interaction
					...
					.then(async () => {
						...
						roleCollector.on('collect', async (i: StringSelectMenuInteraction) => {
							let selection: string;
							await i.guild.roles
								.fetch(i.values[0].split(' ')[1])
								.then((r: Role) => {
							...
							embed
								...
								.setDescription('find')
								.setColor('Yellow')
								.setFields([{ name: 'Role name', value: selection, inline: true }]);
							response = 'find ' + selection;
							await interaction
								.editReply({
									components: [selectionRow],
...
function getChoices(): string {
	...
	for (const [key, value] of Object.entries(choices) as [string, string][]) {
		switch (key) {
			case 'welcomeChannel':
				value !== undefined
					? (results[0] = '**Welcome channel**: ' + (value.startsWith('find') ? value.split(' ')[1] : value))
					: '';
				break;
			case 'logChannel':
				value !== undefined
					? (results[1] = '**Log channel**: ' + (value.startsWith('find') ? value.split(' ')[1] : value))
					: '';
				break;
			case 'memberRole':
				value !== undefined
					? (results[2] = '**Member role**: ' + (value.startsWith('find') ? value.split(' ')[1] : value))
					: '';
				break;
			case 'muteRole':
				value !== undefined
					? (results[3] = '**Muted role**: ' + (value.startsWith('find') ? value.split(' ')[1] : value))
					: '';
				break;
			default:
				break;
