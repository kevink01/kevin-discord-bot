...
export const command: Command = {
  ...
  examples: [
    ...
    {
      command: "emote delete :example:",
      description: "Removes :example: from guild",
    },
    {
      command: "emote rename :example: ex",
  ...
  execute: (message, client, args) => {
    ...
    switch (args[0].toLowerCase()) {
      case "add":
        ...
        try {
          message.guild.emojis
            .create({ attachment: url, name: emoji.name })
            .then(async (newEmoji: GuildEmoji) => {
              await message.channel
                .send(`Successfully added ${newEmoji} to the server!`)
      ...
      case "delete":
      case "del":
        let delEmote = message.guild.emojis.cache.find(
          (e) => e.id === emoji.id
        );
        if (!delEmote) {
          resultPrint(message, "Couldn't find emoji. Try again.", 2000);
        ...
        try {
          message.guild.emojis.delete(delEmote).then(async () => {
            await message.channel
              .send(`Successfully deleted ${emoji.name}`)
              .then(async (m: Message) => {
                await delay(2000);
                m.delete();
              });
        ...
        break;
      case "rename":
        let renameEmote = message.guild.emojis.cache.find(
          (e) => e.id === emoji.id
        );
        if (!renameEmote) {
          resultPrint(message, "Couldn't find emoji. Try again.", 2000);
        ...
        try {
          message.guild.emojis
            .edit(renameEmote, { name: args[2] })
            .then(async (newEmoji: GuildEmoji) => {
              await message.channel
                .send(`New emoji name for emoji ${newEmoji}: ${newEmoji.name}`)
                .then(async (m: Message) => {
                  await delay(2000);
                  m.delete();
                });
