...
export const command: Command = {
  ...
  minArgs: 1,
  maxArgs: 1,
  args: [{ argument: "number", required: true }],
  permissions: ["ManageMessages"],
  aliases: ["delete", "del", "purgeChat"],
  examples: [{ command: "bulk 10", description: "Deletes last 10 messages" }],
  execute: async (message, client, args) => {
    ...
    await textChannel.messages.delete(message).then(async () => {
      await textChannel
        .bulkDelete(messageCount)
        .then(async (collection: Collection<string, Message>) => {
          textChannel
            .send(`Deleted ${collection.size} messages`)
