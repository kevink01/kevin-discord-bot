...
export const command: Command = {
    ...
    examples: [
        ...
        { command: 'unmute userID he apologized', description: 'Mutes user  with reason \'he apologized\'' }
    ],
    execute: async (message, client, args) => {
        ...
        user.disableCommunicationUntil(null, reason).then(async (user: GuildMember) => {
            await message.channel.send(`Unmuted ${user.user.username}`).then(async (m: Message) => {
                await delay(2000);
                m.delete();
            }).catch(console.error);
