...
export const command: Command = {
  ...
  examples: [
    {
      command: "mute @user 15m",
      description: "Mutes user for 15 minutes with mention",
    },
    {
      command: "mute userID 1h",
      description: "Mutes user for 1 hour with user id",
    },
    {
      command: "mute userID 4h he was bad",
      description: "Mutes user for 4 hours with reason 'he was bad'",
    },
  ],
  execute: async (message, client, args) => {
    ...
    if (user.communicationDisabledUntilTimestamp > Date.now()) {
      resultPrint(
        message,
        `User is already muted until ${user.communicationDisabledUntil}`,
        2000
      );
      return;
    }
    ...
    switch (timeInterval) {
      ...
      default:
        resultPrint(
          message,
          "Unknown time format. Please only use 'm', 'h', or 'd'",
          2000
        );
        return;
    }
    ...
    user
      .disableCommunicationUntil(date, reason)
      .then(async (user: GuildMember) => {
        await message.channel
          .send(`Muted ${user.user.username} until ${new Date(date)}`)
