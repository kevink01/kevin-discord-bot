...
export const event: Event = {
  ...
  execute: async (client, interaction: Interaction) => {
    if (interaction.isChatInputCommand()) {
      const slashCommand: SlashCommand = client.slashCommands.get(
        interaction.commandName
      );
      if (!slashCommand) {
        await interaction
          .reply({ content: "Couldn't find command", fetchReply: true })
      ...
      switch (interaction.customId) {
        case "Help Menu":
          ...
          if (command.minArgs) {
            embed.addFields({
              name: "Minimum args",
              value: command.minArgs.toString(),
              inline: true,
            });
          }
          if (command.maxArgs) {
            embed.addFields({
              name: "Minimum args",
              value: command.maxArgs.toString(),
              inline: true,
            });
          }
          if (command.args) {
          ...
          if (command.examples) {
            ...
            for (let i = 0; i < command.examples.length; i++) {
              examplesStr +=
                ...
                "**: " +
                command.examples[i].description +
                "\n";
            }
